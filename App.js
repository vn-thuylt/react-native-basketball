...
import Hoop from './src/components/Hoop'
...
const FLOOR_Y = 11
const HOOP_Y = height - 227
const NET_HEIGHT = 6
const NET_WIDTH = 83
const NET_Y = height - 216
const NET_X = (width / 2) - (NET_WIDTH / 2)
...
export default class App extends Component {
  ...
  updateCollisionVelocity(nextState, ball, netBorder) {
    ...
    const ballScalarNormalAfter = (ballScalarNormal * (ball.mass - netBorder.mass) + 2 * netBorder.mass * netScalarNormal) / (ball.mass + netBorder.mass)
  ...
  handleCollision(nextState) {
    if (nextState.lifecycle !== LC_FALLING && nextState.lifecycle !== LC_BOUNCING) {
      return
    }
  ...
  updateScale = (nextState) => {
    if (nextState.lifecycle === LC_BOUNCING || nextState.lifecycle === LC_RESTARTING || nextState.lifecycle === LC_RESTARTING_FALLING) {
      return
    }
  ...
  handleRestart = (nextState) => {
    ...
    if ((outOfScreen === true)
      || ((nextState.lifecycle === LC_FALLING
        || nextState.lifecycle === LC_BOUNCING) && (nextState.y + (radius * nextState.scale * 2)
          < FLOOR_Y + radius * -2))) {
